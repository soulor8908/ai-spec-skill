...
export async function generateProject(opts) {
    ...
    const experimentalChoices = [];
    for (const key of ['backend', 'db', 'frontend', 'auth', 'ci']) {
...
function buildNextSteps(opts, depsInstalled) {
    const steps = [
 ...
 */
export function cleanupPartial(outDir) {
    if (existsSync(outDir)) {
