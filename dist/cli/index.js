...
import { validateOptions, GOLDEN_COMBO } from './options.js';
...
async function main() {
    ...
    program
        ...
        .action(async (projectName, cmdOpts) => {
...
async function runGenerate(projectName, cmdOpts) {
    logger.banner();
    // 1. 校验项目名
    if (!projectName) {
    ...
    const cmdStack = {};
    if (cmdOpts.backend)
        cmdStack.backend = cmdOpts.backend;
    if (cmdOpts.db)
        cmdStack.db = cmdOpts.db;
    if (cmdOpts.frontend)
        cmdStack.frontend = cmdOpts.frontend;
    if (cmdOpts.contract)
        cmdStack.contract = cmdOpts.contract;
    if (cmdOpts.auth)
        cmdStack.auth = cmdOpts.auth;
    if (cmdOpts.ci)
        cmdStack.ci = cmdOpts.ci;
    ...
    const opts = {
        ...
        out_dir: cmdOpts.out ?? `./${projectName}`,
        stack,
    ...
    try {
        ...
        if (result.deps_installed)
            logger.info('依赖：已安装');
        if (result.git_inited)
            logger.info('Git：已初始化');
...
async function resolveStackInteractive(initial) {
