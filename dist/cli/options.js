 ...
 */
export function isExperimental(category, value) {
    const opts = STACK_OPTIONS[category];
    return opts.find((o) => o.value === value)?.experimental === true;
}
/**
 * 统计 stack 中 experimental 选项数量。
 */
export function countExperimental(stack) {
    let n = 0;
    for (const key of Object.keys(STACK_OPTIONS)) {
        if (isExperimental(key, stack[key]))
            n++;
    }
    return n;
}
...
export function validateOptions(opts) {
    ...
    for (const key of Object.keys(STACK_OPTIONS)) {
        ...
        const valid = STACK_OPTIONS[key].map((o) => o.value);
        if (!valid.includes(value)) {
            errors.push(`stack.${key}="${value}" 不合法，可选：${valid.join(', ')}`);
            continue;
        }
        // experimental 警告
        const opt = STACK_OPTIONS[key].find((o) => o.value === value);
        if (opt?.experimental) {
            warnings.push(`stack.${key}="${value}" 在 MVP 期为 experimental，可能存在缺陷`);
        }
    }
    return { errors, warnings, inferred: stack };
}
