...
// ============ 1. 项目根文件 ============
export function renderRootFiles(opts) {
    return [
...
function renderPackageJson(opts) {
    ...
    if (opts.stack.backend.endsWith('-ts'))
        workspaces.push('packages/contracts', 'apps/api');
...
function renderAiSpecConfig(opts) {
    const isTs = opts.stack.backend.endsWith('-ts');
    const cfg = {
...
export function renderAiSpec(opts) {
    const writes = [];
...
export function renderContracts(opts) {
    const writes = [];
    const isTs = opts.stack.backend.endsWith('-ts');
    if (isTs) {
        writes.push({
...
// ============ 8. docs/ 目录占位 ============
export function renderDocs(opts) {
    return [
        {
            path: 'docs/.gitkeep',
            content: '',
