...
`;
// ============ 6. scripts/ ============
export function renderScripts(opts) {
    return [
        {
...
// ============ 7. .github/workflows/ ============
export function renderCi(opts, warnings) {
    if (opts.stack.ci !== 'github-actions') {
        // experimental CI 防护：显式警告
        warnings.push(`ci="${opts.stack.ci}" 为 experimental，未生成 CI 配置（experimental 适配器防护，建议 1）`);
        return [{
...
function renderGithubActions(opts) {
...
// ============ 9. 测试设置（P1-4 阶段保证全绿） ============
export function renderTestSetup(opts) {
    const writes = [];
