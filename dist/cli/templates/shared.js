 ...
 */
export function loadAdapterFileOrThrow(type, id, fileName) {
 ...
 * 检查适配器文件是否存在（不抛错）。
 */
export function adapterFileExists(type, id, fileName) {
    const path = join(__dirname, '..', '..', 'src', 'adapters', type, id, 'files', fileName);
 ...
 */
export function renderAdapterTemplate(template, opts) {
    let result = template;
 ...
 */
export function walkCopy(srcDir, destDir) {
    const ops = [];
