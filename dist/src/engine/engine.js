...
export class RuleEngine {
    ...
    async run() {
        ...
        for (const rule of targetRules) {
            ...
            if (rule.id === 'META-003' || rule.id === 'META-004')
                continue;
    ...
    checkMeta003() {
        ...
        for (const rule of this.rules) {
            ...
            const plugin = this.findPluginForRule(rule.id);
            if (!plugin) {
                violations.push(`META-003 声明漂移：规则 ${rule.id} 声称 plugin_required=true，但无 plugin 注册 supported_rules 含此 ID`);
            }
        }
        return violations;
    }
        ...
        for (const plugin of this.plugins.values()) {
            for (const id of plugin.supported_rules) {
                if (!declaredIds.has(id)) {
                    violations.push(`META-004 反向缺口：plugin ${plugin.id} 注册了 supported_rules="${id}"，但 kernel/rules 无对应规则定义`);
                }
            }
        }
        return violations;
     ...
     * 查找支持某规则 ID 的 plugin。
     */
    findPluginForRule(ruleId) {
        for (const plugin of this.plugins.values()) {
            if (plugin.supported_rules.includes(ruleId))
                return plugin;
        }
        return undefined;
    }
