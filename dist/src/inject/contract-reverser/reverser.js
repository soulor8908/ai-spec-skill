 ...
 */
function computeAccuracy(confidence, hasRequest, hasResponse) {
    if (confidence >= 0.9 && hasRequest && hasResponse)
        return 'high_confidence';
    if (confidence >= 0.7 && (hasRequest || hasResponse))
        return 'partial';
    return 'inferred';
}
/**
 * 逆向生成 OpenAPI 契约。
 */
export function reverseApi(rootDir, profile) {
    const warnings = [];
...
// ============ TS 路由提取 ============
function extractTsRoutes(content, file) {
    const calls = [];
...
// ============ Spring Boot 路由提取 ============
function extractSpringRoutes(content, file) {
    const calls = [];
...
// ============ OpenAPI 构建 ============
function buildOpenApi(endpoints, profile) {
    const paths = {};
    ...
    const overallAccuracy = allHighConfidence ? 'high_confidence' : anyInferred ? 'inferred' : 'partial';
    return {
        openapi: '3.0.3',
        info: {
...
// ============ Markdown 报告 ============
function renderMarkdown(endpoints, warnings, accuracySummary) {
    const lines = [];
    ...
    if (endpoints.length === 0) {
        ...
        lines.push('|---|---|---|---|---|---|');
        for (const ep of endpoints) {
            lines.push(`| ${ep.method} | ${ep.path} | ${ep.handler_file ?? '-'} | ${ep.confidence.toFixed(2)} | [${ep.accuracy}] | ${ep.notes ?? ''} |`);
        }
    }
...
function collectSourceFiles(rootDir, profile) {
    const files = [];
    const exts = profile.language === 'java'
        ? ['.java']
        : profile.language === 'python'
            ? ['.py']
            : ['.ts', '.tsx', '.js', '.jsx'];
