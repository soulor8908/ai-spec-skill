 ...
 */
export function planInjection(rootDir, profile, config) {
    const writes = [];
 ...
 * 执行注入（写文件 + 备份）。
 */
export function executeInjection(rootDir, plan) {
    if (plan.dry_run) {
    ...
    writeFileSync(join(plan.backup_dir ?? join(rootDir, '.ai-spec', 'backup', 'latest'), 'injection-manifest.json'), JSON.stringify(manifest, null, 2) + '\n');
    return { written, backups };
}
/**
 * 回滚最近一次注入。
 */
export function rollbackInjection(rootDir) {
...
function applySeverityOverrides(content, config) {
...
function makeBackupDir(rootDir) {
...
function findRuleFile(rootDir, ruleId) {
    ...
    for (const name of readdirSync(rulesDir)) {
        ...
        const doc = parseYaml(content);
        if (doc.rules?.some((r) => r.id === ruleId))
            return path;
    }
    return undefined;
}
function loadReadiness(rootDir) {
...
function renderMarkdownPlan(writes, impact, config, profile) {
    const lines = [];
    ...
    if (profile.backend)
        lines.push(`- 后端：${profile.backend.label} (置信度 ${profile.backend.confidence})`);
    if (profile.frontend)
        lines.push(`- 前端：${profile.frontend.label}`);
    if (profile.db)
        lines.push(`- 数据库：${profile.db.label}`);
    if (profile.orm)
        lines.push(`- ORM：${profile.orm.label}`);
    if (profile.test_runner)
        lines.push(`- 测试：${profile.test_runner.label}`);
    lines.push('');
    lines.push('## 2 · 影响范围');
    lines.push('');
