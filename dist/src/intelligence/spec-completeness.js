 ...
 */
export function scoreSpec(specPath) {
    ...
    for (const section of SECTIONS) {
        ...
        if (!found) {
            ...
            suggestions.push(`章节 ${section.id} 字段填充率仅 ${Math.round(fieldResult.coverage * 100)}%，缺：${fieldResult.missing.join(', ')}`);
        }
    }
    // 总分归一化到 0-100
...
function findSection(content, keywords) {
...
function checkFields(content, fields) {
    const missing = [];
    let found = 0;
...
function renderReport(specPath, total, sections, suggestions) {
    const lines = [];
    ...
    for (const s of sections) {
        lines.push(`| ${s.section_id} | ${s.found ? '✓' : '✗'} | ${Math.round(s.field_coverage * 100)}% | ${s.score} |`);
    }
    lines.push('');
