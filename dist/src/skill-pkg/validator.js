...
export function validateSkillManifest(manifest) {
    ...
    // 1. 必填字段
    if (!pkg.name)
        errors.push('[package] name 必填');
    if (!pkg.version)
        errors.push('[package] version 必填');
    if (!pkg.description)
        warnings.push('[package] description 为空（建议填写以便 search）');
    if (!pkg.author)
        warnings.push('[package] author 为空');
    ...
    if (pkg.name && !/^[a-z][a-z0-9-]*$/.test(pkg.name) && !/^@[a-z][a-z0-9-]*\/[a-z][a-z0-9-]*$/.test(pkg.name)) {
        errors.push(`[package] name "${pkg.name}" 不合法（须为简单名 user-mgmt 或命名空间 @ns/name，全小写 + 连字符）`);
    }
    // 3. version 语义化
