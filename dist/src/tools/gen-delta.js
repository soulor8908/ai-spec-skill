...
function parseArgs() {
    ...
    const opts = { from: '', to: 'HEAD', tag: false };
...
function generateDelta(opts) {
    ...
    if (endpoints.length) {
        ...
        if (endpoints.length > 20)
            lines.push(`- ... ${endpoints.length - 20} more`);
        lines.push('');
    }
    if (contracts.length) {
    ...
    if (tests.length) {
        ...
        for (const c of tests.slice(0, 20))
            lines.push(`- [${c.status}] ${c.path}`);
        if (tests.length > 20)
            lines.push(`- ... ${tests.length - 20} more`);
        lines.push('');
    }
    if (reviews.length) {
    ...
    if (opts.tag && opts.round != null) {
        try {
            ...
            console.warn(`⚠️  打 tag 失败: ${e.message}`);
        }
    }
    return outPath;
}
...
generateDelta(opts);
//# sourceMappingURL=gen-delta.js.map